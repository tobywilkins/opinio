...
curl -X POST -d '{"auth_token:"}' http://localhost:3000/api/v1/polls -H Content-Type:application/json -v


curl -i \
  -X POST \
  -F caption_a="thailand" \
  -F caption_b="India"  \
  -F title="where should I go" \
  -F auth_token=bryxUHsG1UNydoo655Dz \
  -F image_a=@spec/photos/beach.jpg \
  -F image_b=@spec/photos/mountains.jpg \
  -H Content-Type:application/json -v
  http://localhost:3000/api/v1/polls \
